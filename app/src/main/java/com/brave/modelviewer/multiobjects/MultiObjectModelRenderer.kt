package com.brave.modelviewer.multiobjects

import android.opengl.GLES20
import android.opengl.GLSurfaceView
import android.opengl.Matrix
import com.brave.modelviewer.Floor
import com.brave.modelviewer.Light
import com.brave.modelviewer.Model
...
*/
class MultiObjectModelRenderer(private val model: List<Model>?) : GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(unused: GL10) {
        ...
        if (model != null) {
            for (i in model.indices) {
                model[i].draw(viewMatrix, projectionMatrix, light)
            }
        }
    }
    ...
    override fun onSurfaceCreated(unused: GL10, config: EGLConfig) {
        ...
        if (model != null) {
            for (i in model.indices) {
                model[i].init(MODEL_BOUND_SIZE)
                floor.setOffsetY(model[i].floorOffset)
            }
        }
    }
